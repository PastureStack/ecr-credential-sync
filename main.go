...
func main() {
	...
	platformURL, err := requiredPlatformEnvironment("PLATFORM_URL", "CATTLE_URL")
	if err != nil {
		log.Fatal(err)
	}
	platformAccessKey, err := requiredPlatformEnvironment("PLATFORM_ACCESS_KEY", "CATTLE_ACCESS_KEY")
	if err != nil {
		log.Fatal(err)
	}
	platformSecretKey, err := requiredPlatformEnvironment("PLATFORM_SECRET_KEY", "CATTLE_SECRET_KEY")
	if err != nil {
		log.Fatal(err)
	}
...
func requiredPlatformEnvironment(name, compatibilityName string) (string, error) {
	if value := strings.TrimSpace(os.Getenv(name)); value != "" {
		return value, nil
	}
	if value := strings.TrimSpace(os.Getenv(compatibilityName)); value != "" {
		return value, nil
	}
	return "", fmt.Errorf("%s is required", name)
}
